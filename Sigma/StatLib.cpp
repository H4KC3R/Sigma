...
int MNK1pow (double *x, double *y, int n0, int nk, double *a,double *b)
{
	...
	// бля ну тут прошлый разработчик ёбу дал, какой костыль получился
	if ((n != 0) && fabs(-x1*x1/n+x2) != 0.0){
		bb=(+y1x1-x1*y1/n)/(-x1*x1/n+x2);
		aa=y1/n-bb*x1/n;
		*(a)= aa;
		*(b)= bb;
		if(fabs(-x1*x1/n+x2)>0.001)
			return 0;
		else
            return 1;
	}
...
int MNKW(double *x, double *y, double *p, int M,int N, double *a)
{
	...
	filem=fopen("new_mnk_weight.txt","wt");
...
int MNKAndr()
{
	double **Matr,**Matrh, **Matr_res;
	Matr=new double *[N];
	Matrh=new double *[M+1];
        Matr_res =new double *[N];

	for (int i=0;i<N;  i++)   Matr[i]=new double [M+1];
	for (int i=0;i<M+1;i++)   Matr[i]=new double [N];
